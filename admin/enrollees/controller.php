...
function doConfirm(){
		...
		$_SESSION['SY'] = $sy;
		 ...
		 $sql = "SELECT * FROM `subject` s, `course` c 
					WHERE s.COURSE_ID=c.COURSE_ID AND s.COURSE_ID=".$studcourse['ENROLL_LEVEL']." ";
 			...
 			while ($row = mysqli_fetch_array($resSubject)) {
					...
					$studentsubject->SY 		= $studcourse['ENROLL_YEAR'];
			...
			$sql = "SELECT * FROM `tblvalues`";
			$mydb->setQuery($sql);

			$cur = $mydb->loadResultList();

			foreach ($cur as $result) {
				$sql = "INSERT INTO `studentvalues` (`IDNO`, `VALUE_ID`, `SY`, `ATTEMP`)
							VALUES ('".$_GET['IDNO']."', '".$result->VALUE_ID."', '".$studcourse['ENROLL_YEAR']."', 1 )";
				$res = mysqli_query($mydb->conn,$sql) or die(mysqli_error($mydb->conn));

				$sql = "INSERT INTO `corevalues` (`IDNO`, `VALUE_ID`)
							VALUES ('".$_GET['IDNO']."', '".$result->VALUE_ID."')";
				$res = mysqli_query($mydb->conn,$sql) or die(mysqli_error($mydb->conn));

			}
				
			$sql = "INSERT INTO `schoolyr`
				               (`AY`, `COURSE_ID`, `IDNO`, `CATEGORY`, `DATE_RESERVED`, `DATE_ENROLLED`, `STATUS`)
				        VALUES ('".$studcourse['ENROLL_YEAR']."','".$studcourse['ENROLL_LEVEL']."','".$_GET['IDNO']."','ENROLLED','".date('Y-m-d')."','".date('Y-m-d')."','New');";
				$res = mysqli_query($mydb->conn,$sql) or die(mysqli_error($mydb->conn));



			$query = "SELECT * FROM `tblstudent` WHERE `COURSE_ID`=".$studcourse['ENROLL_LEVEL'];
			...
			if ($numrow > 40) {
				...
				$student->COURSE_ID = $studcourse['ENROLL_LEVEL'];
				$student->SYEAR = $studcourse['ENROLL_YEAR'];
				$student->ENROLL_LEVEL = 0;
				$student->ENROLL_YEAR = "";
			                      ...
			                      // 	 $mydb->setQuery($query);
				                     ...
				                     //  foreach ($cur as $result) { 
				                      	...
				                      	if (file_exists($rows['SUBJ_ID'])) {
											...
											$studentsubject->SY 		= $_SESSION['SY'];
											$studentsubject->updateSubject($rows->SUBJ_ID,$_POST['IDNO']);
				                      	}else{
