...
<div class="row">
      <div class="col-lg-12">
            ...
            <h1 class="page-header">New Enrollees</h1>
       		...
       		<div class="col-lg-6" >
       			<img style="float:right;" src="<?php echo web_root; ?>img/logocircle1.png" >
       		</div>
       		</div>
        	<!-- /.col-lg-12 -->
   		 </div>
	 		    <form action="controller.php?action=delete" Method="POST">  
				  ...
				  <tbody>
				  	...
				  	// `STATUS`, `AGE`, `NATIONALITY`, `RELIGION`, `CONTACT_NO`, `HOME_ADD`, `EMAIL`, `student_status`
				  		$mydb->setQuery("SELECT * FROM `tblstudent` s, course c WHERE s.ENROLL_LEVEL=c.COURSE_ID AND ENROLL_LEVEL!=0");
						...
						foreach ($cur as $result) {
				  		...
				  		//echo '<td>' .$age.'</td>';
				  		echo '<td>'. $result->CURRENT_ADD.'</td>';
				  		// echo '<td>' . $result->EMAIL.'</a></td>';
				  		//echo '<td>'. $result->COURSE_NAME.'</td>';
						if($result->COURSE_MAJOR != "N/A"){
							echo '<td>'.$result->COURSE_NAME.' - Grade '.$result->COURSE_LEVEL.' ('.$result->COURSE_MAJOR.')</td>';
						}else{
							echo '<td>'.$result->COURSE_NAME.' - Grade '.$result->COURSE_LEVEL.' ('.$result->ENROLL_YEAR.')</td>';
						}
				  		// if($result->student_status=='New'){
							echo '<td align="center" > <a title="Confirm" href="index.php?view=view&IDNO='.$result->IDNO.'"  class="btn btn-info btn-xs  ">Confirm<span class="fa fa-info-circle fw-fa"></span></a>
							</td>';
